...
<?php $component->withAttributes([]); ?>
    <section class="text-gray-900 bg-gray-200">
        <div class="p-4">
            <h1 class="text-3xl">Modifier le produit numéro <?php echo e($product['id']); ?></h1>
        </div>
        <div class="px-3 py-4 flex justify-center">
            <form method="POST" action="/backOffice/updateProduct/<?php echo e($product['id']); ?>" enctype="multipart/form-data">
                <?php echo csrf_field(); ?>

                <div class="mb-4">
                    <label for="name" class="block text-sm font-medium text-gray-700">Name:</label>
                    <input type="text" name="name" id="name" class="mt-1 p-2 w-full border rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm" value="<?php echo e($product['name'] ?? ''); ?>" required>
                </div>
                
                <div class="mb-4">
                    <label for="description" class="block text-sm font-medium text-gray-700">Description:</label>
                    <textarea name="description" id="description" rows="4" class="mt-1 p-2 w-full border rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm" value="<?php echo e($product['description'] ?? ''); ?>"><?php echo e($product['description'] ?? ''); ?></textarea>
                </div>
                
                <div class="mb-4">
                    <label for="price" class="block text-sm font-medium text-gray-700">Price (€):</label>
                    <input type="number" name="price" id="price" step="0.01" class="mt-1 p-2 w-full border rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm" value="<?php echo e($product['price'] ?? ''); ?>" required>
                </div>
                <div class="mb-4">
                ...
                <?php if($product['img']): ?>
                    <div class="mb-4">
                        <img src="<?php echo e($product['img']); ?>" class="w-32 h-32 rounded-md object-cover" alt="Product Image">
                    </div>
                <?php endif; ?>
...
<?php endif; ?>
<?php /**PATH C:\laragon\www\projet-exam\resources\views//backOffice/products/updateProduct.blade.php ENDPATH**/ ?>